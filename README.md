...
6. **Model Evaluation**:
   Two evaluation methods are executed on the test set. First is the accuracy of the generated sentence compared with ground-truth sentence. But since the ambiguity of the prefix, so we can have many result from the same prefix sequence, for example:
   input: fitwchinme
   ground truth: first two character input method
   another possible output: final twitch channel in media
   
   So another evaluation is to see how the sentence perform well as the real sentence. I assess how likely a given sentence is to be generated by the GPT-2 language model. The lower the negative log-loss value returned, the better, as it indicates higher model accuracy and thus a higher probability of the sentence being naturally and appropriately generated.
   input: "I am a student."
   Evaluated Score: -4.268132209777832
   ```sh
